...
class DoomFire:
    ...
    def calc_fire(array):
        """
        Funcao geradora que foi isolada para poder acelerar com numba
        """
    ...
    def draw_fire(self):
        """
        Funcao que pinta a tela de acordo com os valores do fire array.
        """
    ...
    def get_fire_array(self):
        """
        Retorna um array com os indices das cores. Atencao nao retorna as cores no formato rgb
        retorna uma lista com indexes dos valores da palleta 'COLORS'
        """
        fire_array = [
            [0 for i in range(self.FIRE_WIDTH)] for j in range(self.FIRE_HEIGHT)
        ]
    ...
    def draw_pallete(self):
        ...
        for i, color in enumerate(self.palette):
            pg.draw.rect(
                self.app.screen,
                color,
                (i * size, self.app.HEIGHT // 2, size - 5, size - 5),
            )
    ...
    def update(self):
        self.fire_array = self.do_fire(
            self.fire_array, self.FIRE_WIDTH, self.FIRE_HEIGHT
        )
